(() => {
  if (typeof playerJerseyNumber === "function") return;
  ...
  createPlayer = function(side, index, name, team, era) {
    ...
    player.visualScale = Math.max(.88, Math.min(1.2, 1 + (height - 78) * .022));
    player.radius = 20 + (player.visualScale - 1) * 8;
    player.speed += (78 - height) * 1.8;
    player.number = jersey(name, team.id);
    return player;
  };
  const baseDrawPlayer = drawPlayer;
  drawPlayer = function(player) {
    const scale = player.visualScale || 1;
    if (Math.abs(scale - 1) < .01) return baseDrawPlayer(player);
    const x = player.x, y = player.y;
    ctx.save(); ctx.translate(x, y); ctx.scale(scale, scale); player.x = 0; player.y = 0;
    try { baseDrawPlayer(player); } finally { player.x = x; player.y = y; ctx.restore(); }
  };
  const showIntro = () => {
    const intro = document.querySelector("#matchupIntro");
    if (!intro || !game) return;
    const home = game.homeTeam, away = game.awayTeam;
    intro.style.setProperty("--intro-home", home.primary); intro.style.setProperty("--intro-away", away.primary);
    [["Home",home,game.homeEra],["Away",away,game.awayEra]].forEach(([side,team,era]) => {
      ...
      document.querySelector(`#intro${side}Name`).textContent = teamTitle(team).toUpperCase();
      document.querySelector(`#intro${side}Lineup`).textContent = team[era].join(" / ");
    });
    intro.classList.remove("show"); void intro.offsetWidth; intro.classList.add("show"); game.deadBall = 3.15;
  };
  const baseBeginMatch = beginMatch;
  beginMatch = function() { baseBeginMatch(); showIntro(); };
  const showPeriod = title => {
    const graphic = document.querySelector("#periodGraphic"); if (!graphic) return;
    document.querySelector("#periodGraphicTitle").textContent = title;
    graphic.classList.remove("show"); void graphic.offsetWidth; graphic.classList.add("show");
  };
  const baseEndQuarter = endQuarter;
  endQuarter = function() {
    const previousQuarter = game.quarter; baseEndQuarter();
    if (!game || !game.running || game.quarter === previousQuarter) return;
    game.deadBall = 1.85;
    showPeriod(game.quarter > 4 ? "OVERTIME" : `${["FIRST","SECOND","THIRD","FOURTH"][game.quarter - 1]} QUARTER`);
  };
  updateStick = function(event) {
    ...
    knob.style.transform = `translate(${x}px,${y}px)`; touchVector.x = x / limit; touchVector.y = y / limit;
  };
})();
